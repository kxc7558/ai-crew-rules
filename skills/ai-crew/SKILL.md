---
name: ai-crew
description: 给项目装上 AI 施工队规矩：四层分层架构骨架、项目启动关卡（先搜开源+分层评估钩子）、多 AI 派工单。当用户说"装 AI 施工队规矩""给项目装规矩""ai-crew"或要求建立分层架构/开源优先/多 AI 分工制度时使用。
---
...
3. 询问用户（一次问完）：
   ...
   - 除了 Claude Code 还在用哪些 AI 编程工具？各自的额度情况（免费/订阅）？（决定派工单分工）
...
1. 向用户确认各工具的额度格局（第 0 步已问）
2. 基于 `templates/rules/ai-dispatch.md` 生成定制版派工单，写入 `~/.claude/rules/common/ai-dispatch.md`：
   - 按用户实际的工具清单填工种表（额度多的当施工队，额度稀缺最强的当主攻手+质检员，免费的当杂活工）
   - 若用户只用 Claude Code 一个工具，生成简化版：仅保留防打架三原则和知识库规范，删除多工具工种表
3. 若用户使用 Codex：把同一套厂规（分层、开源优先、git 节点、知识库）写入 `~/.codex/AGENTS.md`（已有内容走合并）
