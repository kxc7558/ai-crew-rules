...
- 觉得太灵敏 → 删宽泛词（「做项目」「开发」「scaffold」）
- 觉得太迟钝 → 加你的常用说法
- 纯英文环境 → 保留 `\bnew (project|app|feature|…)\b` 和 `add a feature` 那几条，删掉中文部分

改完记得跑一次回归测试，避免悄悄改坏某个case：
...
同一文件里的 `GATE` 常量就是注入给 AI 的完整指令文本，可按团队规范改写。注意保持两条核心：①开源优先硬门槛 ②分层评估。
...
方向规则不变：展示层 → 逻辑层 → 数据层，只准向下调。改完记得同步更新各层 README 和 `CLAUDE.md` 里的层名。
...
宪法内容（分层地图、职责表、接口清单）和台账机制都是纯 Markdown，任何工具通用；差异只在文件放的位置。
...
欢迎 PR：更多语言的关键词表、更多项目类型的层名映射、其他工具的适配说明。保持每个文件单一职责、总量精简。
